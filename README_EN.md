...
- **Athena renders SVG diagrams in chat**: the AI now draws flowcharts, structure and sequence diagrams as SVG by default (no more ASCII art) and the chat renders them as real graphics with a collapsible source view; SVG is allow-list sanitized so scripts, event handlers and foreignObject are stripped
- **Draggable notification center**: it starts in the bottom-right corner and can be dragged anywhere (clamped to the viewport and re-clamped on resize), with its position remembered locally — no longer covering the send button
- **More tolerant SVG rendering**: handles every fence flavour (```svg / ```xml / ```html / no language tag, plus truncated output with an unclosed fence)
