...
- **Athena 直接渲染 SVG 图形**：AI 画流程图、结构图、时序图时默认改用 SVG（不再用 ASCII 字符画），聊天中直接渲染成图形并可展开查看源码；SVG 经过白名单消毒，脚本/事件/foreignObject 会被清除
- **通知中心可拖动（并修复了拖动不生效、展开后无法收起的问题）**：默认停在右下角，可按住拖到任意位置（自动限制在视口内、窗口缩放时重新收拢），位置本地记忆，不再遮挡发送按钮
- **SVG 渲染更耐造**：兼容各种代码块写法（```svg / ```xml / ```html / 无语言标记，以及输出被截断导致围栏未闭合）
...
本项目基于 **Apache License 2.0** 开源，完整条款见 [LICENSE](LICENSE)，归属声明见 [NOTICE](NOTICE)。
