...
- **Athena가 SVG 도형을 채팅에 직접 렌더링**: 순서도·구조도·시퀀스 다이어그램을 기본적으로 SVG로 그리며(ASCII 아트 대신) 채팅에서 실제 그림으로 표시하고 소스를 펼쳐 볼 수 있습니다. SVG는 화이트리스트 방식으로 정제하여 스크립트·이벤트 핸들러·foreignObject를 제거합니다
- **알림 센터 드래그 가능**: 기본은 오른쪽 아래이며 원하는 위치로 끌 수 있고(뷰포트 안으로 제한, 창 크기 변경 시 재조정) 위치가 로컬에 저장됩니다. 더 이상 전송 버튼을 가리지 않습니다
- **SVG 렌더링 내구성 향상**: 모든 코드 블록 표기(```svg / ```xml / ```html / 언어 표기 없음, 출력이 잘려 닫히지 않은 경우 포함)를 지원
